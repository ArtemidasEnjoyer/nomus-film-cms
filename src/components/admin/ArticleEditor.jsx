...
export function ArticleEditor({ 
  ...
  return (
    <form onSubmit={handleSubmit} class="space-y-8 bg-white dark:bg-gray-800 p-10 rounded-2xl shadow-xl border border-brown/5 relative">
      ...
      <div class={`overflow-hidden transition-all duration-500 ease-in-out ${showHelp ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div class="p-8 bg-brown/5 dark:bg-brown/10 rounded-2xl text-sm border border-brown/10 grid grid-cols-2 gap-6 text-[var(--text-primary)]">
          <div class="space-y-3">
      ...
      <div class="space-y-6">
        ...
        <textarea value={excerpt} onInput={(e) => updateField('excerpt', e.target.value)} class="w-full p-6 rounded-xl bg-gray-50 dark:bg-gray-900 border border-brown/10 h-24 outline-none text-[var(--text-primary)]" placeholder={t.summary} required />
        <textarea value={content} onInput={(e) => updateField('content', e.target.value)} class="w-full p-6 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-brown/10 h-80 outline-none font-mono text-sm text-[var(--text-primary)]" placeholder={t.contentLabel} />
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <select value={category} onChange={(e) => updateField('category', e.target.value)} class="p-4 rounded-xl bg-gray-50 dark:bg-gray-900 border border-brown/10 font-bold text-sm outline-none text-[var(--text-primary)]">
          ...
          <div class="flex gap-2">
            <input value={image} onInput={(e) => updateField('image', e.target.value)} class="flex-1 p-4 rounded-xl bg-gray-50 dark:bg-gray-900 border border-brown/10 text-sm outline-none text-[var(--text-primary)]" placeholder={t.coverImage} />
            <button type="button" onClick={() => fileInputRef.current.click()} class="p-4 bg-brown/10 rounded-xl text-xl text-[var(--text-primary)]">🖼️</button>
            <input type="file" ref={fileInputRef} onChange={handleImageUpload} class="hidden" accept="image/*" />
          </div>
        </div>
      </div>
      <div class="flex gap-4">
        <button class="flex-1 py-5 bg-gray-900 text-white dark:bg-pastelGreen dark:text-gray-900 font-black rounded-2xl">{editingId ? t.update : t.publish}</button>
        {editingId && <button type="button" onClick={resetForm} class="px-8 py-5 bg-gray-100 dark:bg-gray-700 rounded-2xl font-bold text-[var(--text-primary)]">{t.cancel}</button>}
      </div>
