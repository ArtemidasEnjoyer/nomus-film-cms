    ...
    <div class="space-y-10">
      ...
      <div class="flex flex-col md:flex-row gap-4">
        <div class="flex-1 relative">
          <input type="text" placeholder={t.searchPlaceholder} value={searchTerm} onInput={(e) => setSearchTerm(e.target.value)} class="w-full pl-12 pr-4 py-4 rounded-xl bg-white dark:bg-gray-800 border border-brown/10 outline-none text-[var(--text-primary)]" />
          <span class="absolute left-4 top-1/2 -translate-y-1/2 opacity-30 text-xl">🔍</span>
        </div>
        <select value={filterType} onChange={(e) => setFilterType(e.target.value)} class="p-4 rounded-xl bg-white dark:bg-gray-800 border border-brown/10 font-bold text-sm text-[var(--text-primary)]">
        ...
        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} class="p-4 rounded-xl bg-white dark:bg-gray-800 border border-brown/10 font-bold text-sm text-[var(--text-primary)]">
          <option value="newest">{t.newest}</option><option value="oldest">{t.oldest}</option><option value="title">{t.byTitle}</option>
        </select>
      </div>
      <div class="grid grid-cols-1 gap-4">
        {displayArticles.map(a => (
          <div key={a.id} class="flex items-center justify-between p-8 bg-white dark:bg-gray-800 rounded-2xl border border-brown/10 shadow-sm">
            <div class="flex gap-6 items-center">
              <div class="w-16 h-16 rounded-xl overflow-hidden hidden sm:block"><img src={a.image} class="w-full h-full object-cover" /></div>
              <div>
